            ...
            for cc in range(len(tt)):
                ...
                with torch.no_grad(): 
                 ...
                 for cat in range(18)], axis=1)
                    ...
                    #predictive = Predictive(model=mm, guide=gg, num_samples=60, return_sites=(['pred']))
                    #ss = predictive([torch.from_numpy(time).type(dtype), torch.from_numpy(dnpr_dummies).type(dtype), torch.from_numpy(genealogy).type(dtype), torch.from_numpy(bth).type(dtype), torch.from_numpy(baseline).type(dtype), torch.from_numpy(sex).type(dtype)])
                    #pred = np.percentile(ss['pred'].numpy()[:, :, 0], [0.025, 0.5, 0.975], axis=0).T
                    ...
                    #pp.extend(np.concatenate((pred, p_dnpr, p_gene, p_bth, p_base, p_dnpr_sub), axis=1)[None, :, :].tolist())                 
...
with h5py.File(file, 'w') as f:
        f.create_dataset('main', data=ll_main, maxshape=(None, 9), compression='lzf')
        f.create_dataset('pred', data=ll_pred, maxshape=(None, 25, 22), compression='lzf')
        f.create_dataset('post', data=ll_post, maxshape=(None, 7), compression='lzf')
        f.create_dataset('postpred', data=ll_post2, maxshape=(None, 2, 22), compression='lzf')
...
jupyter nbconvert --to script /home/people/alexwolf/projects/CancerRisk/scripts/m1/predict.ipynb --output /home/people/alexwolf/projects/CancerRisk/scripts/m1/predict

/services/tools/anaconda3/5.3.0/bin/python3.6 /home/people/alexwolf/projects/CancerRisk/scripts/m1/predict.py $VAR1
' >> run.sh

for ii in 107 115 143 235 343 406 422 614 692 869 906; do qsub -v VAR1=$ii run.sh; done
