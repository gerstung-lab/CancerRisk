...
sys.path.append(ROOT_DIR + 'projects/CancerRisk/others/qrisk')
...
for _, [dd, dd_add] in tqdm.tqdm(enumerate(zip(ukb_iterator, ukb_iterator_add))):
        ...
        age = ((np.datetime64('2015-01-01')-birthdate).astype(float)/365).astype(int).tolist()[0]
        bmi = ((weight*100+75)/((height*100+170)/100)**2).astype(int)
        alcohol_cat4 = np.asarray([(alcohol[-1]==6), np.logical_or((alcohol[-1] ==4), (alcohol[-1] ==5)), np.logical_or((alcohol[-1] ==2), (alcohol[-1] ==3)), (alcohol[-1] ==1)]).astype(int)
        smoke_cat = np.asarray([(smoking[-1]==0), (smoking[-1]==-3), (smoking[-1]==-10), (smoking[-1] ==2), (smoking[-1] ==1)]).astype(int)
        
        b_chronicpan=d_codes[0, 753]
        b_copd=d_codes[0, 657]
        town = 0
        
        b_endometrial =  np.maximum(d_codes[0, 976], d_codes[0, 976])
        b_type2 = d_codes[0, 264]
        c_hb = 0
    
        fh_breastcancer = np.any(gene_breast>=1).astype(int)
        fh_gicancer = np.any(gene_colorectal>=1).astype(int)
        fh_ovariancancer = 0
        fh_prostatecancer = np.any(gene_prostate>=1).astype(int)
        
        new_abdodist = d_codes[0, np.where(disease_codes[:, 0]=='R19')[0]]
        new_abdopain = d_codes[0, np.where(disease_codes[:, 0]=='R10')[0]]
        new_appetiteloss = d_codes[0, np.where(disease_codes[:, 0]=='R63')[0]]
        new_breastlump = d_codes[0, np.where(disease_codes[:, 0]=='N63')[0]]
        new_breastpain = d_codes[0, np.where(disease_codes[:, 0]=='N64')[0]]
        new_breastskin = d_codes[0, np.where(disease_codes[:, 0]=='N64')[0]]
        new_dysphagia = d_codes[0, np.where(disease_codes[:, 0]=='R13')[0]]
        new_gibleed = d_codes[0, np.where(disease_codes[:, 0]=='K92')[0]]
        new_haematuria = d_codes[0, np.where(disease_codes[:, 0]=='R31')[0]]
        new_haemoptysis = d_codes[0, np.where(disease_codes[:, 0]=='R04')[0]]
        new_heartburn = d_codes[0, np.where(disease_codes[:, 0]=='R12')[0]]
        new_imb = d_codes[0, np.where(disease_codes[:, 0]=='N92')[0]]
        new_indigestion = d_codes[0, np.where(disease_codes[:, 0]=='K30')[0]]
        new_necklump = d_codes[0, np.where(disease_codes[:, 0]=='R22')[0]]
        new_nightsweats = d_codes[0, np.where(disease_codes[:, 0]=='R61')[0]]
        new_pmb = d_codes[0, np.where(disease_codes[:, 0]=='N95')[0]]
        new_postcoital = d_codes[0, np.where(disease_codes[:, 0]=='N93')[0]]
        new_rectalbleed = d_codes[0, np.where(disease_codes[:, 0]=='K62')[0]]
        new_vte = d_codes[0, np.where(disease_codes[:, 0]=='N93')[0]]
        new_weightloss = d_codes[0, np.where(disease_codes[:, 0]=='R63')[0]]
        new_testespain = d_codes[0, np.where(disease_codes[:, 0]=='N50')[0]]
        new_testicularlump = d_codes[0, np.where(disease_codes[:, 0]=='N44')[0]]
        
        s1_bowelchange = d_codes[0, np.where(disease_codes[:, 0]=='R19')[0]]
        s1_bruising = d_codes[0, np.where(disease_codes[:, 0]=='R23')[0]]
        s1_constipation = d_codes[0, np.where(disease_codes[:, 0]=='K59')[0]]
        s1_cough = d_codes[0, np.where(disease_codes[:, 0]=='R05')[0]]
        s1_impotence = np.asarray([0])
        s1_nocturia = d_codes[0, np.where(disease_codes[:, 0]=='R35')[0]]
        s1_urinaryfreq = d_codes[0, np.where(disease_codes[:, 0]=='R35')[0]]
        s1_urinaryretention = d_codes[0, np.where(disease_codes[:, 0]=='R33')[0]]
    
        
        score = qrisk(sex, age, bmi, alcohol_cat4, smoke_cat, b_chronicpan, b_copd, b_endometrial, b_type2, c_hb, fh_breastcancer, fh_gicancer, fh_ovariancancer, fh_prostatecancer, new_abdodist, new_abdopain, new_appetiteloss, new_breastlump, new_breastpain, new_breastskin, new_dysphagia, new_gibleed,
...
s1_impotence, s1_nocturia, s1_urinaryfreq, s1_urinaryretention)
        ...
        with h5py.File(ROOT_DIR + 'projects/CancerRisk/data/main/predictions/qrisk_' + str(run_id) + '.h5', 'a') as f:
            f.create_group(str(eid[0]))
            f[str(eid[0])].create_dataset('pred', data=score.astype(float), maxshape=(1, 22), compression='lzf')
